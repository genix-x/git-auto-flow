#!/bin/bash
# git-cz-auto.sh - Wrapper pour automatisation git cz avec Gemini
...
# Banner
echo -e "${BLUE}🤖 Git CZ Auto - Powered by Gemini AI${NC}"
echo -e "${BLUE}======================================${NC}"

# Charge le fichier .env si il existe
if [ -f .env ]; then
    export $(grep -v '^#' .env | xargs)
    echo -e "${GREEN}✅ Clé API Gemini chargée depuis .env${NC}"
fi
...
# Vérifie qu'on est dans un repo git avec des changements stagés
if ! git diff --cached --name-only | grep -q .; then
    echo -e "${YELLOW}⚠️  Aucun fichier stagé détecté${NC}"
    echo -e "${BLUE}💡 Suggestions:${NC}"
    echo -e "   ${YELLOW}git add .${NC}              # Stager tous les fichiers"
    echo -e "   ${YELLOW}git add <fichier>${NC}      # Stager un fichier spécifique"
    echo -e "   ${YELLOW}git status${NC}             # Voir l'état des fichiers"
    exit 1
fi

# Trouve le script Python
SCRIPT_DIR="$(dirname "$0")"
PYTHON_SCRIPT="$SCRIPT_DIR/git-cz-auto.py"
